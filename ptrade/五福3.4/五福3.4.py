...
一、定时任务框架
----------------
- 09:00 晨间流水线：持仓检查 → 回撤监控 → 流动性阈值计算 → 动态池更新 → 固定池过滤 → 合并池
- 13:10 午后流水线：震荡期退出检查 → 震荡期进入检查 → 动量计算 → 卖出执行 → 买入执行
- 15:10 收盘重置：重置止损标志 → 更新震荡期统计
- 分钟级止损：盘中实时监控固定比例止损和当日跌幅止损

二、ETF池构建机制
----------------
1. 固定ETF池（约120只）：涵盖大宗商品、海外指数、港股、宽基指数、行业主题ETF
2. 动态ETF池（最多100只）：从全市场ETF中筛选高流动性品种，按行业分组取头部
3. 流动性过滤：剔除日均成交额低于全市场阈值/20000的ETF
4. 合并池：固定池与动态池合并后作为候选池

三、动量筛选逻辑（四步法）
--------------------------
第一步：计算所有ETF的动量得分并排序
  - 动量得分 = 年化收益率 × R²（加权线性回归斜率）
  - 正常期使用25天周期，震荡期切换为21天短期动量

第二步：应用多层过滤条件
  - 动量得分区间过滤（0-5分或0-6分）
  - R²拟合度过滤（阈值>0.4）
  - 成交量比过滤（近5日均量比<1.8，排除放量异常）
  - 短期风控过滤（近3日单日跌幅<3%）
  - 溢价率过滤（实盘可用）
  - 动态滤波器过滤（拉普拉斯/高斯滤波器斜率判断趋势）

第三步：构建候选池
  - 取前10名中动量得分≥第N名得分×0.9的ETF（N为目标持仓数）
...
进入震荡期条件（任一触发）：
  - 乖离率过大（价格偏离20日均线>8%）
  - RSI超买回落（前日RSI>70，当日RSI<65且下降）
  - 今日触发止损

退出震荡期条件（任一触发）：
  - 从近20日低点上涨≥4%
  - 连续2日企稳信号（回撤<2%+价格回升+RSI回升）
  - 震荡期满20个交易日
...
五、止损机制
------------
1. 分钟级固定比例止损：持仓亏损达5%时触发（成本价×95%）
2. 分钟级当日跌幅止损：当日跌幅达5%时触发（昨收×95%）
3. 止损触发后记录标志，午后流水线检查是否进入震荡期

六、交易执行
------------
卖出：不在目标ETF池中的持仓全部卖出（检查涨跌停/停牌）
买入：按目标持仓数分配资金，等额买入目标ETF（检查涨跌停/停牌）
防御模式：无符合条件的ETF时买入银华日利货币ETF（511880.SS）
